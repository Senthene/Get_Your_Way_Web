...
function dxfToSvg(dxfString)
{
    ...
    function dxfObjectToSvgSnippet(dxfObject, i)
    {
        function getLineSvg(x1, y1, x2, y2, i)
        {
            return '<path id="'+i+'" data-info="<div>State: Hawaii</div><div>Capital: Honolulu</div>" fill="#D3D3D3" d="M{0},{1} {2},{3}"/>\n'.format(x1, y1, x2, y2);
        }
        ...
        switch (dxfObject.type) {
            case 'LINE':
                ...
                return getLineSvg(dxfObject.x, dxfObject.y, dxfObject.x1, dxfObject.y1, i);
            case 'CIRCLE':
        ...
        if (counter++ % 2 === 0) {
            ...
            if (groupCode === 'blockName' && value === 'ENTITIES') {
                ...
                if (groupCode === 'entityType') {  // New entity starts.
                    if (object.type) {
                        i = i+1;
                        svg += dxfObjectToSvgSnippet(object, i);
                    }
    ...
    svg = '<svg {0} id="map" version="1.1" xmlns="http://www.w3.org/2000/svg">\n' +
          '<g '.format(pixelToMillimeterConversionRatio) +
